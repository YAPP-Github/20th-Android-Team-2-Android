...
class SettingsActivity : BaseActivity<ActivitySettingsBinding>(R.layout.activity_settings) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.user.flowWithLifecycle(lifecycle = this.lifecycle)
            ...
            .onEach {
                when (it) {
                    is SettingRepository.Settings.Error -> {
                        showToast("유저 정보가 없습니다.")
                    }
                    is SettingRepository.Settings.Success -> {
                        binding.viewInit(it)
                    }
                    else -> Unit
                }
    ...
    private fun ActivitySettingsBinding.viewInit(value: SettingRepository.Settings.Success) {
        ...
        tvVersion.text =
            packageManager.getPackageInfo(this@SettingsActivity.packageName, 0).versionName
        tvCreatedAt.text = getString(
            R.string.setting_created_at,
            LocalDateTime.parse(value.createAt)?.format(DateTimeFormatter.ofPattern("yy.MM.dd"))
        )
    }

    private fun ActivitySettingsBinding.setOnClickListener() {
        ivBack.setOnSingleClickListener(200) {
            finish()
        }

        viewPolicy.setOnSingleClickListener(200) {
        ...
        viewUser.setOnSingleClickListener {
            startActivity(
                withDrawNavigator.intent(this@SettingsActivity)
                    .putExtra(EMAIL, binding.tvUserId.text)
            )
        }
    }
}
